...
public class CliInstallTask implements Callable<Void> {
	...
	public static CliInstallTask newClientLifegemTask(Path destination) {
		return new CliInstallTask(Installer.newPlatformInstaller(destination, CLIENT_LIFEGEM));
	}

	public static CliInstallTask newClientAsceticTask(Path destination) {
		return new CliInstallTask(Installer.newPlatformInstaller(destination, CLIENT_ASCETIC));
	}

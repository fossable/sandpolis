...
//! Regular agents cannot manage snapshots because, unless the filesystem itself
//! has snapshot support, the agent is likely to produce inconsistent results (which
...
/// ## Create Snapshot
///
/// If there exist no previous snapshots, the agent first determines the appropriate
...
/// block hashes. A single worker thread reads blocks from the disk and compares
/// their hashes against the block hashes retrieved from the server. If the hashes
/// do not match, the block is passed into a send queue to be egressed to the
/// server.
///
/// ## Apply Snapshot
///
/// If there exists a previous snapshot for the disk, the agent initiates a stream
/// of block hashes. A single worker thread reads blocks from the disk and passes
/// their hashes into a send queue to be egressed to the server.
///

...
use crate::input::{Key, KeyboardControllable, MouseButton, MouseControllable};

use libc::c_int;
use x11::xlib::{Display, XCloseDisplay, XGetPointerMapping, XOpenDisplay};
use crate::input::libxdo_sys::{self, xdo_t, CURRENTWINDOW};
...
impl MouseControllable for EnigoXdo {
    ...
    fn mouse_down(&mut self, button: MouseButton) -> crate::input::ResultType {
        if self.xdo.is_null() {
            return Ok(());
        }
        unsafe {
...
impl KeyboardControllable for EnigoXdo {
    ...
    fn key_down(&mut self, key: Key) -> crate::input::ResultType {
        if self.xdo.is_null() {
            return Ok(());
        }
    ...
    fn key_sequence_parse_try(&mut self, sequence: &str) -> Result<(), crate::input::dsl::ParseError>
    where
        Self: Sized,
    {
        crate::input::dsl::eval(self, sequence)
    }
}

...
use anyhow::{Result as ResultType, bail};
...
pub fn try_close_session() {
    ...
    if let Some(rdp_info) = &*rdp_info {
        ...
        if use_screencast_portal() && rdp_info.is_support_restore_token {
            close = true;
        }
    }
    if close {
...
impl PipeWireRecorder {
    pub fn new(capturable: PipeWireCapturable) -> ResultType<Self> {
        ...
        // If `use_screencast_portal()` is false, it means using remote_desktop_portal,
...
// If `use_screencast_portal()` is true, then `screencast_portal::start` is called.
// Otherwise, `remote_desktop_portal::start` is called.
//
// `screencast_portal` supports restore_token and persist_mode (so the user is
// not re-prompted by the permission dialog on every session) if the portal
// version is greater than or equal to 4, but provides no input injection.
// `remote_desktop_portal` also provides input injection, but no restore_token
// or persist_mode.
pub(crate) fn use_screencast_portal() -> bool {
    true
}
...
pub fn try_fix_logical_size(shared_displays: &mut Vec<crate::capture::Display>) {
    if !use_screencast_portal() {
        return;
    }
    ...
    for sd in shared_displays.iter_mut() {
        if let crate::capture::Display::WAYLAND(d) = sd {
...
pub fn fill_displays(
    ...
    shared_displays: &mut Vec<crate::capture::Display>,
) -> ResultType<()> {
    if !use_screencast_portal() {
        return Ok(());
    }
    ...
    shared_displays.iter_mut().next().map(|d| {
        if let crate::capture::Display::WAYLAND(d) = d {
            d.0.primary = true;
        }
    });
...
fn save_positions_to_cache(displays: &Arc<Displays>, shared_displays: &Vec<crate::capture::Display>) {
    let restore_token = config::LocalConfig::get_option(RESTORE_TOKEN_CONF_KEY);
    if restore_token.is_empty() {
        return;
    }

use crate::capture::{quartz, Frame, Pixfmt};
...
impl crate::capture::TraitCapturer for Capturer {
    fn frame<'a>(&'a mut self, _timeout_ms: std::time::Duration) -> io::Result<Frame<'a>> {
        match self.frame.try_lock() {
            Ok(mut handle) => {
                ...
                match frame {
                    Some(mut frame) => {
                        crate::capture::would_block_if_equal(&mut self.saved_raw_data, frame.inner())?;
...
impl<'a> crate::capture::TraitPixelBuffer for PixelBuffer<'a> {
    fn data(&self) -> &[u8] {
        &*self.frame
    }

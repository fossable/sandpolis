...
public class AttributeValueGenerator extends DefaultTask {
	...
	private void generateArrayAttributeValue(ArrayTypeName type) throws IOException {
		var av = TypeSpec //
				...
				.superclass(ParameterizedTypeName.get(ClassName.get(DATA_PACKAGE, "AttributeValue"), type));
...

		{
			...
			var method = MethodSpec.methodBuilder("clone") //
					.addModifiers(PUBLIC) //
					.addAnnotation(Override.class) //
					.returns(ParameterizedTypeName.get(ClassName.get(DATA_PACKAGE, "AttributeValue"), type)) //
		...
		// Output the class
		JavaFile.builder(getProject().getName() + ".data", av.build())
	...
	private void generateListAttributeValue(ClassName type) throws IOException {
		...
		var av = TypeSpec //
				...
				.superclass(ParameterizedTypeName
						.get(ClassName.get("com.sandpolis.core.instance.data", "AttributeValue"), listType));
...

		{
			...
			var method = MethodSpec.methodBuilder("clone") //
					.addModifiers(PUBLIC) //
					.addAnnotation(Override.class) //
					.returns(ParameterizedTypeName.get(ClassName.get(DATA_PACKAGE, "AttributeValue"), listType)) //
		...
		if (convertableTypes.contains(type)) {
			valueField.addAnnotation( //
					AnnotationSpec.builder(ClassName.get("javax.persistence", "Convert"))
							.addMember("converter", "$T.class", ClassName.get(DATA_PACKAGE,
	...
	public void generateConverter(ClassName type) throws IOException {
...

		{
			// Add instance field
			var field = FieldSpec
					.builder(ClassName.get(DATA_PACKAGE, type.simpleName() + "Converter"), "INSTANCE", PUBLIC, STATIC,
							FINAL) //
